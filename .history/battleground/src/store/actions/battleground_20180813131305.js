...
export const pushPickedSquares = (props) =>{
    let value = [];
    for(let i = 0; i <6; i++){
        let shuffled = shuffle(props.battleground.squareIndex);
        let selected = shuffled.slice(0, props.battleground.numberOfTerrains[i]);
        value.push(selected);
    }
    return{
        type: 'PICKED_SQUARES',
